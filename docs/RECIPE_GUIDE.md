...
レシピに画像を追加する場合、Markdownファイルと**同名**の画像ファイルを同じディレクトリに配置してください。
...
❌ **間違った例:**
- frontmatter に `image:` フィールドを書く必要はありません
- 異なるファイル名（例: `risotto-image.png`）は認識されません
...
### 注意点
- 画像ファイル名はMarkdownファイル名と**完全に一致**させる（拡張子を除く）
- 画像は自動的に最適化され、適切なサイズで表示されます
- 一覧ページ: 600x400px
- 詳細ページ: 1200x800px
- OGP画像: 1200x600px
