...
- `==...==` は **手順の要点に対する修飾** にのみ使用する（行為そのものではなく修飾語句にかける）
- 例: `==冷蔵庫で6〜12時間==抽出する`
...
**[TAGGING.md](./docs/agents/TAGGING.md)** - タグ設計ガイド（分類、命名、統合・禁止ルール）
...
**設計品質:**
- [ ] DRY原則に準拠しているか（同じ情報を複数箇所に記述していないか）
- [ ] 規約ベース vs 明示的記述のバランスは適切か
- [ ] データとアセットが適切に分離されているか
- [ ] スキーマ設計が拡張性を考慮しているか
...
**メンテナンス性:**
- [ ] ファイル名変更時にfrontmatterの更新が必要か（DRY原則違反）
- [ ] 新規レシピ追加時の手順が複雑すぎないか
- [ ] ドキュメントが更新されているか
...
1. **PR URLを受け取る**
2. **変更内容を確認**: `gh pr view`, `gh pr diff`
3. **問題点を特定**: 上記観点に基づいて分析
4. **改善提案を作成**:
   - 問題点の説明
   - 改善案（複数あれば比較）
   - メリット・デメリット
5. **ユーザー確認**: 改善方針について合意
6. **実装**: 同一ブランチまたは新規ブランチ
7. **動作確認**: 開発サーバーで確認
8. **コミット・push**: 適切なコミットメッセージで

### リファクタリング判断基準

**即座にリファクタリングすべき:**
- DRY原則違反（同じ情報が複数箇所に存在）
- 明らかなパフォーマンス問題
- スキーマと実装の不整合
...
**配置場所:**
- レシピ画像: `src/content/recipes/{category}/{recipe-name}.{ext}`
- Markdownファイルと同名で配置（拡張子を除く）

**命名規則:**
- Markdownファイル名と完全一致させる
- frontmatterに`image`フィールドは不要（規約ベース）

**対応フォーマット:**
- 優先順: png > jpg/jpeg > webp > avif
